...
class IndicLID():
    ...
    def __init__(self, input_threshold = 0.5, roman_lid_threshold = 0.6):
        ...
        absolute_path = "/root/abhinav/pipeline/IndicSpeechFlow/IndicLID/Inference/" #Path where models are stored 
        
        self.IndicLID_FTN_path = os.path.join(absolute_path, 'models/indiclid-ftn/model_baseline_roman.bin')
        self.IndicLID_FTR_path = os.path.join(absolute_path, 'models/indiclid-ftr/model_baseline_roman.bin')
        self.IndicLID_BERT_path = os.path.join(absolute_path, 'models/indiclid-bert/basline_nn_simple.pt')
