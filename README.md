...
Primary LLM: ChatGroq with model llama-3.1-8b-instant for task selection, script generation, and prompt verification.
Secondary LLM: ChatGroq with model deepseek-r1-distill-llama-70b for prompt checker iterations.
...
Operating System: Linux (tested on Ubuntu, paths assume /raid/ filesystem).
...
pip install pandas langchain langchain-groq langgraph jiwer langchain-experimental


Set Up Groq API Key:

Obtain a Groq API key from Groq.
Set the environment variable:export GROQ_API_KEY="gsk_vTHbkVGZYKr"
...
Verify that utility_functions2.py is present in the same directory and contains functions like transcribe_folder_to_csv, process_folder_vad, etc.
If missing, implement or obtain these functions from the original source.


Prepare Input Data:

Audio Directory: Place audio files (e.g., .wav) in a directory like /raid/ganesh/pdadiga/chriss/agent/AI_Agent_Final/time-task/QC2/audios/.
Ground Truth CSV: Create a CSV (e.g., /raid/ganesh/pdadiga/chriss/test_data/new_transcriptions.csv) with columns like Filename and ground_truth.



Example Input CSV
Filename,ground_truth
audio1.wav,नमस्ते, आप कैसे हैं?
audio2.wav,यह एक परीक्षा है
...
CSV files are saved in the specified audio directory or CSV directory, e.g.:
language_verification.csv
ctc_scores.csv
wer.csv
normalized_list.csv
...
character_list.csv:
Filename,ground_truth,character_list
audio1.wav,नमस्ते, आप कैसे हैं?,आएककैसनमसतेह
audio2.wav,यह एक परीक्षा है,आएकपरीक्षयह


vocab_list.csv:
Filename,ground_truth,vocab_list
audio1.wav,नमस्ते, आप कैसे हैं?,['आप', 'कैसे', 'नमस्ते', 'हैं']
audio2.wav,यह एक परीक्षा है,['एक', 'परीक्षा', 'यह', 'है']


wer.csv:
Reference,Hypothesis,WER
नमस्ते आप कैसे हैं,नमस्ते आप कैसे हैं,0.0
यह एक परीक्षा है,यह एक परिक्षा है,0.25



Technical Details
Task Dependencies
The pipeline enforces strict dependencies:
...
The character_agent and vocab_agent dynamically detect transcription columns (Transcription, Ground_Truth, ground_truth) case-insensitively.
If no valid column is found, an error is logged, and the task returns a failure message.
This addresses issues with column mismatches (e.g., expecting Ground Truth but finding ground_truth).

Pandas Availability

The pipeline ensures pandas is available by importing it explicitly in generated scripts.
A global check_dependencies function (not present in your code but recommended) could be added to verify pandas installation:def check_dependencies():
    try:
        import pandas
    except ImportError:
        logging.error("Pandas not installed. Please install it using 'pip install pandas'.")
        raise ImportError("Pandas is required for this pipeline.")



QC Check Skipping

QC tasks (19, 20, 21) are only included if explicitly mentioned in the prompt (e.g., "corruption", "extension", "sample rate") or if audio_dir is provided and relevant.
The select_tasks function enforces this by filtering out QC tasks when inappropriate, preventing unnecessary execution.

Error Handling

File Validation: Checks for valid audio_dir and ground_truth_csv before processing.
Exception Handling: Each agent catches and logs exceptions, returning error messages in the state dictionary.
Logging: All operations and errors are logged to pipeline.log with timestamps and severity levels.
...
Invalid Audio Directory or CSV:

Symptom: Logs show Invalid audio directory or Invalid ground truth CSV.
Solution: Verify the paths in the prompt (e.g., /raid/ganesh/pdadiga/chriss/agent/AI_Agent_Final/time-task/QC2/audios/) exist and are accessible.


Column Not Found:

Symptom: Errors like KeyError: 'Ground Truth'.
Solution: Ensure the CSV has a Transcription, Ground_Truth, or ground_truth column (case-insensitive). Update the CSV or adjust the agent to handle custom column names.


LLM Response Parsing Error:

Symptom: Failed to parse topological structure: invalid syntax.
Solution: Check the LLM response in pipeline.log. If malformed, consider increasing max_tokens or refining the prompt in topological_sort_tasks.


QC Tasks Running Unnecessarily:

Symptom: Sample rate check runs without audio_dir.
Solution: The updated select_tasks function prevents this. Verify the prompt does not include QC-related keywords unless intended.



Debugging Tips

Check Logs: Inspect pipeline.log for detailed error messages and stack traces:cat pipeline.log


Enable Verbose Mode: The agent is initialized with verbose=True, providing detailed execution logs in the console.
Test with Small Inputs: Use a small audio directory (2-3 files) and a simple CSV to isolate issues.
Validate Utility Functions: Ensure utility_functions2.py functions are implemented correctly, as they are critical for tasks like transcription and VAD.

Future Improvements

Dependency Validation: Add a check_dependencies function to verify all required libraries at startup.
Custom Column Support: Allow users to specify custom column names in the prompt for greater flexibility.
Task Timeout: Implement timeouts for LLM calls and task execution to prevent hangs on large datasets.
Unit Tests: Develop unit tests for each agent to validate behavior with edge cases (e.g., empty CSVs, missing columns).
LLM Prompt Optimization: Refine prompts in select_tasks and topological_sort_tasks to reduce parsing errors and improve response consistency.

Contributing
Contributions are welcome! To contribute:

Fork the repository.
Create a feature branch (git checkout -b feature/new-task).
...
Contact
For questions or support, contact the repository maintainer at [your-email@example.com] or open an issue on the repository.
